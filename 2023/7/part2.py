...
def hand_value(hand):
    return (hand_type_J(hand), tuple(card_val(c) for c in hand))

def hand_type_J(hand):
    shand = sorted(hand)
    if shand[0] == shand[4]: return 6
    if shand[0] == shand[3] == 'J': return 6
    if shand[1] == shand[4] == 'J': return 6

    def rec(pos):
        if pos == 5: return hand_type(shand)
        if shand[pos] != 'J': return rec(pos+1)
        best = 0
        for z in "23456789TQKA":
            shand[pos] = z
            best = max(best, rec(pos+1))
        shand[pos] = 'J'
        return best
