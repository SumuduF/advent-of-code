...
def solve(task):
    ...
    best = 0
    for sr in range(n):
        best = max(best, count_energized(task, sr, 0, 0, 1))
        best = max(best, count_energized(task, sr, m-1, 0, -1))
    for sc in range(m):
        best = max(best, count_energized(task, 0, sc, 1, 0))
        best = max(best, count_energized(task, n-1, sc, -1, 0))
    return best

def count_energized(task, sr, sc, sdr, sdc):
    n, m = len(task), len(task[0])
